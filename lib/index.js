 ...
 *
 * - GET /dsh-workspace-actions/api/workspaces      → 工作区列表 [{id, path, title}]
 * - GET /dsh-workspace-actions/api/list?path=      → 列一层目录
 ...
 *
 * 安全：仅接受自定义头 X-DSH-Workspace-Actions: 1（跨站请求无法携带自定义头，
    ...
    handler: async (req, res) => {
      ...
      try {
        ...
        if (path === API + '/workspaces') {
          let list = []
          // 优先用 workspaceRegistry（有正式 title 与 id）
          try {
            if (registry && typeof registry.list === 'function') {
              list = registry.list().map((w) => ({ id: String(w.id), path: String(w.path), title: String(w.title) }))
            }
          } catch (e) { list = [] }
          // 兜底：registry 未就绪时，用会话 cwd 分组推导工作区
          if (list.length === 0) {
            try {
              ...
              if (sp && typeof sp.list === 'function') {
                ...
                for (const h of headers) {
                  ...
                  if (cwd && !seen.has(cwd)) {
                    seen.add(cwd)
                    list.push({ id: null, path: cwd, title: basename(cwd) })
                  }
                }
              }
            } catch (e) {}
